...
def reverse(update: Update, context:CallbackContext):
    ...
    try:
        ...
        if response != 400:
            xx = bot.send_message(chat_id, "Searching for results in ALFA Database.."
        ...
        if guess and imgspage:
            xx.edit_text(f"[{guess}]({fetchUrl})\nSearching images in ALFA Database..", parse_mode='Markdown', disable_web_page_preview=True)
        else:
            xx.edit_text("Didn't find a shit in ALFA.")
            return

        images = scam(imgspage, lim)
        if len(images) == 0:
            xx.edit_text(f"[{guess}]({fetchUrl})\n[Visually similar images]({imgspage})"
                          "\nDidn't find a shit in ALFA.", parse_mode='Markdown', disable_web_page_preview=True)
            return
